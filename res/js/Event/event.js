	...
	loadEvent:function(){
		$.ajax({
			...
			success:function(data){
				...
				for(i;i<len;i++){
					html.push('<a style="background-image:url('+data[i].pictureUrl+');background-color:rgb('+data[i].color+')" class="a0 eventBanner'+i+'" href="'+data[i].link+'"></a>');
				}
	...
	myEvent:function(){
		$.ajax({
			...
			success:function(data){
				...
				for(i;i<len;i++){
					html.push('<a class="eventBanner'+i+'" href="/activity/actdetail?id='+data[i].id+'"></a>');
				}
				$("#eventList").html(html.join(''));
			},
			error:function(){cp2y.dialog.clearLoading();}
