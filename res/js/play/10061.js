...
var _={
	...
	playTypes:function(){
		var html=[];
		html.push('<b data="a0" class="onn" data2="一星">一星</b>');
		...
		html.push('<b data="a11" data2="三星直选">三星直选</b>');
		return html.join('');
	}
};
_.a0={
	...
	count:function(){
		...
		units= rb.length;
		CurBets.html(units);
		CurMoney.html(units*2);
	},
	random:function(u){
		var i=0,o=[],ball=[],rb=[];
		for(i;i<10;i++){ball.push(i);}
		i=0;
		for(i;i<u;i++){
...
_.a1={
	...
	count:function(){
		...
		for(i;i<len;i++){
			...
			if(o.eq(i).hasClass("rb2")){rb2.push(o.eq(i));}
			if(o.eq(i).hasClass("rb3")){rb3.push(o.eq(i));}
			if(o.eq(i).hasClass("rb4")){rb4.push(o.eq(i));}
		}
		if(rb1.length>0||rb2.length>0||rb3.length>0||rb4.length>0){s=3;}
		this.setClear(s);
	...
	addContent:function(){
		...
		for(i;i<len;i++){
			...
			if(o.eq(i).hasClass("rb2")){rb2.push(o.eq(i).html());}
			if(o.eq(i).hasClass("rb3")){rb3.push(o.eq(i).html());}
			if(o.eq(i).hasClass("rb4")){rb4.push(o.eq(i).html());}
		}
		...
		var o='<li data_input="'+this.input+'" data_bets="'+units+'" data_code="'+rb4.join('')+'-'+rb3.join('')+'-'+rb2.join('')+'-'+rb1.join('')+'">'+
			'<div><a class="r">'+rb4.join('</a><a class="r">')+'</a> <a class="r">'+rb3.join('</a><a class="r">')+'</a> <a class="r">'+rb2.join('</a><a class="r">')+'</a> <a class="r">'+rb1.join('</a><a class="r">')+'</a></div>'+
			'<p>'+this.playType+'：'+units+'注'+units*2+'元</p>'+
			'<i class="delI" onclick="cp2y.buy.del(this)"></i></li>';
		this.addRecord(o);
		this.clear();
...
_.a2={
	...
	count:function(){
		...
		for(i;i<len;i++){
			...
			if(o.eq(i).hasClass("rb3")){rb3.push(o.eq(i));}
			if(o.eq(i).hasClass("rb4")){rb4.push(o.eq(i));}
			if(o.eq(i).hasClass("rb5")){rb5.push(o.eq(i));}
		}
		if(rb1.length>0||rb2.length>0||rb3.length>0||rb4.length>0||rb5.length>0){s=3;}
		this.setClear(s);
	...
	addContent:function(){
		...
		for(i;i<len;i++){
			...
			if(o.eq(i).hasClass("rb3")){rb3.push(o.eq(i).html());}
			if(o.eq(i).hasClass("rb4")){rb4.push(o.eq(i).html());}
			if(o.eq(i).hasClass("rb5")){rb5.push(o.eq(i).html());}
		}
		...
		var o='<li data_input="'+this.input+'" data_bets="'+units+'" data_code="'+rb5.join('')+'-'+rb4.join('')+'-'+rb3.join('')+'-'+rb2.join('')+'-'+rb1.join('')+'">'+
			'<div><a class="r">'+rb5.join('</a><a class="r">')+'</a> <a class="r">'+rb4.join('</a><a class="r">')+'</a> <a class="r">'+rb3.join('</a><a class="r">')+'</a> <a class="r">'+rb2.join('</a><a class="r">')+'</a> <a class="r">'+rb1.join('</a><a class="r">')+'</a></div>'+
			'<p>'+this.playType+'：'+units+'注'+units*2+'元</p>'+
			'<i class="delI" onclick="cp2y.buy.del(this)"></i></li>';
		this.addRecord(o);
		this.clear();
...
_.a7={
	...
	addContent:function(){
		...
		if(rb1.length==0){rb1.push('-');}
		if(rb2.length==0){rb2.push('-');}
		if(rb3.length==0){rb3.push('-');}
		if(rb4.length==0){rb4.push('-');}
		if(rb5.length==0){rb5.push('-');}
		var o='<li data_input="'+this.input+'" data_bets="'+units+'" data_code="'+rb5.join('')+','+rb4.join('')+','+rb3.join('')+','+rb2.join('')+','+rb1.join('')+'">'+
			'<div><a class="r">'+rb5.join('</a><a class="r">')+'</a> <a class="r">'+rb4.join('</a><a class="r">')+'</a> <a class="r">'+rb3.join('</a><a class="r">')+'</a> <a class="r">'+rb2.join('</a><a class="r">')+'</a> <a class="r">'+rb1.join('</a><a class="r">')+'</a></div>'+
			'<p>'+this.playType+'：'+units+'注'+units*2+'元</p>'+
			'<i class="delI" onclick="cp2y.buy.del(this)"></i></li>';
...
_.a8={
	...
	calcUnits:function(a){
		var b=[];
		$(a).each(function(i,v){
			if(v.length==0 || (v.length==1&&v[0]=='-'))return;
			b.push(v);
		});
		if(b.length<2)return 0;
		var pos=[];
		$(b).each(function(i,v){pos.push(i);});
		var units=0;
	...
	getArray:function(){
		...
		for(i;i<len;i++){
			if(o.eq(i).hasClass("rb5")){w.push(o.eq(i).html());}
			if(o.eq(i).hasClass("rb4")){q.push(o.eq(i).html());}
			if(o.eq(i).hasClass("rb3")){b.push(o.eq(i).html());}
			if(o.eq(i).hasClass("rb2")){s.push(o.eq(i).html());}
			if(o.eq(i).hasClass("rb1")){g.push(o.eq(i).html());}
		}
		return [w,q,b,s,g];
	},
	count:function(){
		var a=this.getArray(),units,s=1;
		if(a[0].length>0||a[1].length>0||a[2].length>0||a[3].length>0||a[4].length>0){s=3;}
		this.setClear(s);
	...
	addContent:function(){
		...
		if(a[0].length==0){a[0].push('-');}
		if(a[1].length==0){a[1].push('-');}
		if(a[2].length==0){a[2].push('-');}
		if(a[3].length==0){a[3].push('-');}
		if(a[4].length==0){a[4].push('-');}
		var o='<li data_input="'+this.input+'" data_bets="'+units+'" data_code="'+a[0].join('')+','+a[1].join('')+','+a[2].join('')+','+a[3].join('')+','+a[4].join('')+'">'+
			'<div><a class="r">'+a[0].join('</a><a class="r">')+'</a> <a class="r">'+a[1].join('</a><a class="r">')+'</a> <a class="r">'+a[2].join('</a><a class="r">')+'</a> <a class="r">'+a[3].join('</a><a class="r">')+'</a> <a class="r">'+a[4].join('</a><a class="r">')+'</a></div>'+
			'<p>'+this.playType+'：'+units+'注'+units*2+'元</p>'+
			'<i class="delI" onclick="cp2y.buy.del(this)"></i></li>';
		this.addRecord(o);
		this.clear();
...
_.a9={
	...
	count:function(){
		...
		if(rb1.length>0||rb2.length>0){s=3;}
		this.setClear(s);
	...
	addContent:function(){
		...
		var o='<li data_input="'+this.input+'" data_bets="'+units+'" data_code="'+rb1.join('')+''+rb2.join('')+'">'+
			'<div><a class="r">'+rb1.join('</a><a class="r">')+'</a><a class="r">'+rb2.join('</a><a class="r">')+'</a></div>'+
			'<p>'+this.playType+'：'+units+'注'+units*2+'元</p>'+
			'<i class="delI" onclick="cp2y.buy.del(this)"></i></li>';
		this.addRecord(o);
		this.clear();
...
_.a10={
	...
	addContent:function(){
		...
		var o='<li data_input="'+this.input+'" data_bets="'+units+'" data_code="'+rb2.join('')+'-'+rb1.join('')+'">'+
			'<div><a class="r">'+rb2.join('</a><a class="r">')+'</a> <a class="r">'+rb1.join('</a><a class="r">')+'</a></div>'+
			'<p>'+this.playType+'：'+units+'注'+units*2+'元</p>'+
			'<i class="delI" onclick="cp2y.buy.del(this)"></i></li>';
		this.addRecord(o);
		this.clear();
...
_.a11={
	...
	count:function(){
		...
		if( rb1.length>0||rb2.length>0||rb3.length>0 ){s=3;}
		this.setClear(s);
	...
	addContent:function(){
		...
		var o='<li data_input="'+this.input+'" data_bets="'+units+'" data_code="'+rb3.join('')+'-'+rb2.join('')+'-'+rb1.join('')+'">'+
			'<div><a class="r">'+rb3.join('</a><a class="r">')+'</a> <a class="r">'+rb2.join('</a><a class="r">')+'</a> <a class="r">'+rb1.join('</a><a class="r">')+'</a></div>'+
			'<p>'+this.playType+'：'+units+'注'+units*2+'元</p>'+
			'<i class="delI" onclick="cp2y.buy.del(this)"></i></li>';
		this.addRecord(o);
		this.clear();
