...
var _={
	...
	playTypes:function(){
		...
		html.push('<b data="a19" class="onn" data2="任选5">任选5</b>');
		...
		html.push('<b data="a14" data2="前3组选">前3组选</b>');
		html.push('<i>胆拖</i>');
		...
		if(b==1){
			if(!$(_this).hasClass("rb")){
				if($(".rb").size()>0){
					...
					$(_this).addClass("rb");
					Choose.children("a").eq($(_this).index()+10).removeClass("bb");
				}
			}else{
			...
			Choose.children("a").eq($(_this).index()-13).removeClass("rb");
		}
		cp2y.buy.count();
	},
	count:function(){
