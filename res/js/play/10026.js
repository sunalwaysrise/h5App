		...
		if(b==1){
			if(!$(_this).hasClass("rb")){
				if($(".rb").size()>3){
					...
					$(_this).addClass("rb");
					Choose.children("a").eq($(_this).index()+34).removeClass("rb2");
				}
			}else{
			...
			Choose.children("a").eq($(_this).index()-37).removeClass("rb");
		}else if(b==2){
			if(!$(_this).hasClass("bb")){
				if($(".bb").size()>0){
					...
					$(_this).addClass("bb");
					Choose.children("a").eq($(_this).index()+9).removeClass("bb2");
				}
			}else{
			...
			Choose.children("a").eq($(_this).index()-16).removeClass("bb");
		}
		this.count();
	},
	count:function(){
